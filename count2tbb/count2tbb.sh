...
# Set DATE
for YYYY in $1 ; do      # Year (from 2015)
  for MM in $2 ; do        # Month
    for DD in $3 ; do      # Day
      for HH in $4  ; do   # Hour
      for MN in $5  ;do    # 00 10 20 30 40 50 # Minute
...
if [ ${CHN} = "VIS" ] || [ ${CHN} = "EXT" ] || [ ${CHN} = "SIR" ];then
   gmt makecpt -Cgray -T0/110/10 -Z > count.cpt
   unit="(%)"
elif [ ${CHN} = "TIR" ];then
   gmt makecpt -Cgray -T200/300/10 -I -Z > count.cpt
   unit="(K)"
fi
imgfn=${YYYY}${MM}${DD}${HH}${MN}.${CHN,,}.${NUM}.fld
gmt xyz2grd grid??.dat -Glittle_endian.gmt -I${resolution}/${resolution} -R85/205/-60/60 -r -di-999. -ZTLf
gmt grdimage little_endian.gmt -JQ140/15 -Ccount.cpt -P -R85/205/-60/60 > ${imgfn}.eps
# gmt pscoast -Ba20g20/a15g15eWSn -J -R -Dh -K -N1/2t7.5_7.5:0/0/255/0 -W1/0/255/0 -O >> ${imgfn}.eps
# echo "140 62 18 0 0 CB Himawari-8 (${para}) ${YYYY}${MM}${DD} ${HH}${MN}UTC" | gmt pstext -R -J -G0/0/0 -O -K -N >> ${imgfn}.eps
# gmt psscale -D7.5/-1/12/0.5h -Ccount.cpt -B10/:"${unit}": -O >> ${imgfn}.eps
gmt psconvert ${imgfn}.eps  -P -A -E300 -Qg -Tt -D../../../data/raw/tc/doksuri/img
# --------------------------------------------------------------
